...
public class tableB {
	...
	// lista righe
	private List<row_B> rows=new ArrayList<row_B>();
	...
	public void modify_projection (Integer projection) { //not used in B
		this.projection.add(projection);
	}
	public void modify_projection_list (List<Integer> projection) {
	...
	*/
	...
	public List<String> showitemsetList () { //not used in B
		return itemset;
	}
	...
	public void modify_projection_and_list2(int i) {  //not used in B
		this.projection.add(i);
		for (row_B r: this.rows)
			for (int a=0; a<=i; a++) {
	...
	public int minimum_length() {
		...
		for (row_B r: rows) if (r.length()<min) min=r.length();
		return min;		
	}
	...
	//metodo per trovare la massima lunghezza delle righe
		public int max_length() {
			int max=0;
			for (row_B r: rows) if (r.length()>max) max=r.length();
	...
	public List<Integer> elements_in_all(int row_number) {
		...
		for (int i=1;i<=row_number;i++) {
			for (row_B r:rows) if (r.showlist_trans().contains(i)) a++;
	...
	//alla fine mai usata	
	public List<Integer> elements_in_all_first(int row_number) { //not used in B
			List<Integer> omnipresent=new ArrayList();
			int a=0;
			boolean stop=true;
			int o= 0;
