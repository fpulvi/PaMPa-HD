...
public class tableC {
	...
	// lista righe
	private List<row_rid> rows=new ArrayList<row_rid>();
	...
	public void modify_projection (Integer projection) { //not used in C
		this.projection.add(projection);
	}
	public void modify_projection_list (List<Integer> projection) { //not used in C
	...
	*/
	...
	public List<String> showitemsetList () {
		return itemset;
	}
	...
	//metodo per restiture la proiezione attuale come stringa con spazi
			public String get_projection_string_spaces () {  //not used in C
				String string1="";
				for (Integer s: projection) string1=string1+" "+s.toString()+",";
				return string1;
	...
	public String get_projection_string_spaces2 () {
		String string1="";
		int n=0;
	...
	public void modify_projection_and_list2(int i) {  //not used in C
		this.projection.add(i);
		for (row_rid r: this.rows)
			for (int a=0; a<=i; a++) {
	...
	public void modify_projection_and_list (List<Integer> l) {
	...
	public int minimum_length() {
		int minimo=rows.get(0).length();
		for (row_rid r: rows) if (r.length()<minimo) minimo=r.length();
		return minimo;		
	}
	...
	//metodo per trovare la massima lunghezza delle righe
		public int max_length() {
			int max=0;
			for (row_rid r: rows) if (r.length()>max) max=r.length();
	...
	public List<Integer> elements_in_all(int row_number) {
		...
		for (int i=1;i<=row_number;i++) {
			for (row_rid r:rows) if (r.showlist_trans().contains(i)) a++;
	...
	//alla fine mai usata	 
	public List<Integer> elements_in_all_first(int row_number) {
			List<Integer> omnipresent=new ArrayList();
			int a=0;
			boolean stop=true;
			int o= 0;
